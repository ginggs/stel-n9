...
void StelApp::preinitCore(QSettings* conf)
{
    confSettings = conf;
    core = new StelCore();
}
...
//    StelLoadingBar loadingBar(splashFileName, QString("N9 %1").arg(STELN9_VERSION), 35, 320, 121);
// #endif
//#endif
    //loadingBar.draw();
...
// Handle touch events
void StelApp::handleTouch(QEvent* event)
{
    // Send the event to every StelModule
    foreach (StelModule* i, moduleMgr->getCallOrders(StelModule::ActionHandleMouseMoves))
    {
