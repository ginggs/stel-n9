				  ...
				  float names_brightness) const
{
    ...
    // Used Stellarium 0.11.1 code as it is quite faster, though it obviously lacks extinction feature
    for (const Star *s=z->getStars();s<end;++s)
    {
        ...
        if (drawer->drawPointSource(sPainter, Vec3d(vf[0], vf[1], vf[2]), tmpRcmag, s->bV, !is_inside) && s->hasName() && s->mag < maxMagStarName && s->hasComponentID()<=1)
        {
            ...
            sPainter->drawText(Vec3d(vf[0], vf[1], vf[2]), s->getNameI18n(), angle, offset, offset, false);
        }
    }
    return;
    ...
    if (withExtinction)
	{
	    ...
	    extinction.forward(&altAz, &extMagShift);
            int extMagShiftStep=qMin((int)floor(extMagShift/k), 1024-mag_steps); // this number muist be equal StarMgr.cpp line 649
